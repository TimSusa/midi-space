...
class Input extends EventEmitter {
  ...
  getReadStream() {
    return midi.createReadStream(this._input)
  }
...
class Output {
  ...
  getWriteStream() {
    return midi.createWriteStream(this._output)
  }
  close() {
  ...
  parseMessage(type, args) {
    ...
    if (type === 'sysex') {
      ...
      if (
        args.length <= 3 ||
        args[0] !== 0xf0 ||
        args[args.length - 1] !== 0xf7
      ) {
        //
        throw new Error(
          'sysex commands should be an array that starts with 0xf0 and end with 0xf7'
        )
      }
