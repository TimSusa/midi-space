...
Arpeggiator.prototype = {
  ...
  addButtons: function () {
    ...
    ctrls.innerHTML = ''
    ctrls.innerHTML +=
      "<p>BPM</p><input type='number' class='bpm' step='1' value='" +
      this.bpm +
      "'>"
    ctrls.innerHTML +=
      "<p>swing</p><input type='number' class='swing' step='0.01' value='" +
      this.swing +
      "'>"
  ...
  clearArpHighlight: function () {
    Array.from(document.querySelectorAll('.arp-cell.active')).forEach((c) =>
  ...
  input: function (evt) {
    ...
    if (keyCode >= 37 && keyCode <= 40) {
      ...
      if (keyCode === 37 && this.column > 0) {
        this.owner.selectCell(this.row, this.column - 1)
      }
      if (keyCode === 38 && this.row > 0) {
        this.owner.selectCell(this.row - 1, this.column)
      }
      if (keyCode === 39) {
        this.owner.selectCell(this.row, this.column + 1)
      }
      ...
      if (!key.trim()) {
        ...
        if (!this.key) {
          ...
          this.element.innerHTML =
            "<span class='arp-cell-label'>" + this.key.note + '</span>'
        }
      }
    }
  },
